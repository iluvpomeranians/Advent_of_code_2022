...
with open(filename, 'r') as f:
    ...
    min_val = 0
    for nodes in updated_graph:
        if next(re.finditer(r'\w', nodes)).group() == 'a':
            shortest_path_length = find_shortest_path(updated_graph, nodes, 'E')
            if min_val == 0:
                min_val = shortest_path_length
            elif shortest_path_length < min_val:
                min_val = shortest_path_length


    print(f"Size: {min_val}")
